...
KEYWORD_SYNONYMS = {
    "מוזיקה": ["מוסיקה", "מוזיקלי", "מופע", "הופעה", "קונצרט", "זמר", "זמרת", "להקה",
                "פסטיבל", "שירה בציבור", "ערב שירה", "ערב שירי", "שירי", "live",
                "דיגיי", "תקליטן", "אמפי", "מארח את", "על הבמה"],
    "ספורט": ["כדורגל", "כדורסל", "מונדיאל", "ריצה", "יוגה", "זומבה", "פילאטיס",
               "התעמלות", "כושר", "אופניים", "שחייה", "טניס", "גודו", "קרטה",
               "ספורטיבי", "מרוץ", "צעדה", "נינגה", "גלישה"],
    "תרבות": ["הצגה", "תיאטרון", "סרט", "מופע", "מוזיאון", "פסטיבל", "טקס",
               "סטנדאפ", "קולנוע", "תערוכה", "ערב שירה", "הרצאה", "שעת סיפור"],
    "ילדים": ["לילדים", "ילדי", "משפחות", "משפחתי", "גימבורי", "גמבורי",
               "בובות", "פעוטות", "תינוקות", "הורים וילדים", "כיתות",
               "הפנינג", "מסיבת קצף", "משחקי חצר", "רך הנולד"],
    "קהילה": ["קהילתי", "קהילת", "שכונות", "שכונתי", "תושבים",
               "התנדבות", "מתנדבים"],
    "יצירה": ["אמנות", "אומנות", "ציור", "פיסול", "קרמיקה", "מלאכה", "יצירתי",
               "נגרות", "לגו", "צילום", "גרפיטי", "קדרות", "עיצוב"],
    "הפגה": ["הפוגה", "מקלט", "מקלטים"],
    "ריקוד": ["מחול", "היפ הופ", "בלט", "רקדנים"],
    "הרצאות": ["הרצאה", "העשרה"],
    "סדנאות": ["סדנת", "סדנה"],
    "קורסים": ["קורס", "הכשרה", "חוג"],
    "הורים": ["הורות", "הורה", "הדרכת הורים", "אמהות"],
    "נשים": ["נשי", "בוקר נשי", "אמהות"],
    "ותיקים": ["האזרח הותיק", "אזרח ותיק", "גיל הזהב", "גמלאים", "ותיקי", "60+"],
}
...
KEYWORD_ALIASES = {
    ...
    "סדנה": "סדנאות", "סדנת": "סדנאות",
    "קורס": "קורסים", "חוגים": "קורסים", "חוג": "קורסים",
...
# שמות מארגנים/גופים שמופיעים כמעט בכל אירוע ומזהמים התאמות קטגוריה:
# "החברה לתרבות פנאי וספורט בת ים" גרמה לכל אירוע (בייבי יוגה, גמילה
# מחיתולים) להיתפס תחת "תרבות" ו"ספורט". מנוקים לפני התאמת טקסט.
# הערכים כאן בצורה מנורמלת (אחרי normalize_text — בלי גרשיים, lowercase).
MATCH_NOISE = [
    "החברה לתרבות פנאי וספורט",
    "החברה העירונית לתרבות פנאי וספורט",
    "תרבות ופנאי בבת ים",
    "אגף לתרבות",
    "אגף תרבות",
    # שמות מקומות — "מרכז קהילתי עופר" גרם לכל סדנה להיתפס תחת "קהילה"
    "מרכז קהילתי",
    "המרכז הקהילתי",
    "מרכז הקהילתי",
]
...
def matchable_text(event):
    """Normalized event text for keyword matching, with organizer noise removed."""
    raw = (event.get("raw_text") or "") + " " + (event.get("title") or "")
    text = normalize_text(raw)
    for noise in MATCH_NOISE:
        text = text.replace(noise, " ")
    return text


def term_in_text(term, text):
    """Word-boundary-aware Hebrew term matching.

    Substring matching caught "מחול" inside "מחולקות" and "סרט" inside "סרטון".
    Hebrew single-word terms now require word boundaries, allowing common
    prefixes (ו/ב/ל/כ/מ/ה/ש) and plural/feminine suffixes (ים/ות/ת/ה/י).
    Multi-word or Latin terms keep plain substring matching.
    """
    if " " in term or re.search(r'[a-z]', term):
        return term in text
...
def _term_pattern(term):
    ...
    if pat is None:
        pat = re.compile(
            r'(?<![א-ת])[ובלכמהש]{0,2}' + re.escape(term) + r'(?:ים|ות|יות|ת|ה|י)?(?![א-ת])'
        )
...
def event_matches_keyword(keyword, event):
    ...
    text = matchable_text(event)
    return any(term_in_text(t, text) for t in keyword_terms(kw))
